...
export function PriceCalculator() {
  ...
  const hasApiKey = !!(
    (import.meta as Record<string, unknown> & { env: Record<string,string> }).env?.VITE_GOOGLE_MAPS_KEY
  );
  ...
  return (
    <div className="bg-white rounded-[24px] shadow-[0_8px_40px_rgba(138,21,56,0.12)] p-6 w-full max-w-md">
      ...
      <div className="flex gap-1 bg-[#F4F1EA] p-1 rounded-[14px] mb-5">
        <button onClick={() => setBookingType('trip')}   className={clsx(tab, bookingType === 'trip'   ? activeTab : inactiveTab)}>مشوار من/إلى</button>
        <button onClick={() => setBookingType('hourly')} className={clsx(tab, bookingType === 'hourly' ? activeTab : inactiveTab)}>حجز بالساعة</button>
      </div>
      ...
      {bookingType === 'trip' ? (
        <div className="space-y-3 mb-5">
          {hasApiKey ? (
            <>
              <input
                type="text"
                placeholder="نقطة الانطلاق (مثل: اللؤلؤة، الننجاري)"
                value={origin}
              ...
              <input
                type="text"
                placeholder="الوجهة (مثل: مطار حمد الدولي، ويست 23)"
                value={destination}
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-[rgba(28,28,28,0.15)] rounded-[12px] px-4 py-3 text-sm focus:outline-none focus:border-[#8A1538] transition-colors"
              />
              <p className="text-[11px] text-[#5A5A5A] mt-1.5">ℹ️ أدخل المسافة يدوياً. لتفعيل الحساب التلقائي أضف VITE_GOOGLE_MAPS_KEY.</p>
            </div>
          )}
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setHours(h => Math.max(1, h - 1))} className="w-9 h-9 rounded-full border border-[rgba(138,21,56,0.3)] text-[#8A1538] font-bold hover:bg-[#8A1538]/8 transition-colors">−</button>
            <span className="text-2xl font-black text-[#1C1C1C] w-8 text-center">{hours}</span>
            <button onClick={() => setHours(h => Math.min(12, h + 1))} className="w-9 h-9 rounded-full border border-[rgba(138,21,56,0.3)] text-[#8A1538] font-bold hover:bg-[#8A1538]/8 transition-colors">+</button>
          </div>
        </div>
      )}
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          {([['standard', 'عادية'], ['luxury', 'فاخرة']] as const).map(([val, label]) => (
            <button key={val} onClick={() => setVehicleClass(val)} className={clsx(chip, vehicleClass === val ? activeChip : inactiveChip)}>{label}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {([['any', 'لا يهم'], ['male', 'ذكر'], ['female', 'أنثى']] as const).map(([val, label]) => (
            <button key={val} onClick={() => setDriverPref(val)} className={clsx(chip, driverPref === val ? activeChip : inactiveChip)}>{label}</button>
          ))}
        </div>
      </div>
      ...
      {!calculated ? (
        <button
          ...
          disabled={loading || (bookingType === 'trip' && !hasApiKey && !manualKm)}
        ...
        <div className="bg-[#FAF8F3] border border-[rgba(138,21,56,0.15)] rounded-[16px] p-5 text-center">
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            <button onClick={handleReset} className="text-xs text-[#5A5A5A] hover:text-[#8A1538] transition-colors">← حساب جديد</button>
          </div>
